...
| Claude Code | `route_supported_cache_unproven` | Claude Code now uses the public `sonnet` alias with `--effort xhigh`; LiteLLM maps that alias to `chatgpt/gpt-5.5` and the local ASGI shim rewrites the Anthropic `system` field before routing. A fresh real smoke or series must refresh provider/cache evidence before any usefulness claim. |
...
- Claude Code: `CLAUDE_LITELLM_COMPRESSION_MODE=off`
...
| Claude Code | `route_supported_cache_unproven` | Current route expectation is `bin/claude-litellm --model sonnet --effort xhigh`, with `sonnet` mapped to `chatgpt/gpt-5.5`. Refresh this row with the next real provider/cache artifact before treating usefulness evidence as current. Older failed artifacts under `tmp/claude-route-proof/` predate the alias rewrite. |
