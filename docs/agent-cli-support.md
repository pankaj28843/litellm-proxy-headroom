...
| Claude Code | Isolated wrapper, provider route gated | `~/.claude-headroom` | `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` plus `--setting-sources project`, generated MCP config, and strict MCP isolation. Current ChatGPT-backed LiteLLM aliases can reject Claude Code system-message shape. | First prove an Anthropic-compatible LiteLLM model route satisfies Claude Code, then run a real Claude Code call series and aggregate provider/cache rows. |
